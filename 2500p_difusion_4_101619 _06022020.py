...
inter_range_off = round(3.51*sigma,13) #defining th einteracting range in terms of debye length
inter_range_on = round(3.50*sigma,13)

################################################ end system parameters ###############################################

########################################## Reaction energy bariers and rates #####################################################
dimer_on_association_e = 1e-1*1e-03*0.2777#math.exp(-3.1234e-20/ (k_b * temp))           
pre_nucleus_on_association_e = 1e-1*1e-03*0.2777#math.exp(-0.09234e-20/ (k_b * temp))
...
dimer_off_association_e = 1.8e04*1e-03*0.2777#math.exp(-3.65e-20/(k_b * temp))
...
second_monomer_attach_e = 1.929e3*1e-03*0.2777#math.exp(-0.0101798e-20/(k_b * temp))
second_pre_nucleus_association_e = 1.929e3*1e-03*0.2777#math.exp(-1.0034e-27 /(k_b * temp))
second_fibril_detach_e = 0#math.exp(-0.10e-26 / (k_b * temp))
...
#                print('dimer on found',M[index], M[irq])
...
#                        break
                        ...
                        if dtest_dist_nucleus_on_low1 >= round(2.0*sigma,13):
                            if dtest_dist_nucleus_on_low1 <= round(3.5*sigma,13):
...
#                                print('added pre nucl from low', dtmp_low, J[dids], dtmp23)
                        ...
                        if dtest_dist_nucleus_on_low2 >= round(2.0*sigma,13):
                            if dtest_dist_nucleus_on_low2 <= round(3.5*sigma,13):
                ...
                if J[ddtr][2] == 0:
                    for ddtw in dlength:
                        if J[ddtw][2] == 1:
                            ...
                            if dtest_dist_sec > round(2.0*sigma,13) and dtest_dist_sec <= round(3.5*sigma,13):
                ...
                if D[edtr][2] == 0:
                    for edtw in elength_1:
                        if D[edtw][2] == 1:
                            ...
                            if etest_dist_sec > round(2.0*sigma,13) and etest_dist_sec <= round(3.5*sigma,13):
...
#    print('dissociating a dimer or prenucleus from routine!!!',n)#, MM[n])
...
#                    monomers3.append(n)
                ...
                if MM[naggregate_left[0]][2] in [33,30,31,32]: #3/12/20
                    ...
                    for nskli in naggregate_left:
                        if (MM[naggregate_left[nskli]][2] == 303):                            
...
def detatch_second(VV,sigma,o, second_fibril_association14,second_fibril_dissociation14,
                   pentamer_on_diffusion14,fibril_dissociation14,fibril_association14,
                   second_monomer_attach14,STOP_TEST14):     # if there are no overlaps with other particles    
    oslope_new_fibril =  o[0][3]
    ofibril_new = o[:]
...
#           fibril_new.append(i) 
    otrans_diff_length = ((len(ofibril_new))**(-1.0/3.0))*2.0*sigma  
    for odd in ofibril_new:        
        VV[odd][0] = VV[odd][0] + otrans_diff_length*math.cos(math.atan(oslope_new_fibril))
        VV[odd][1] = VV[odd][1] + otrans_diff_length*math.sin(math.atan(oslope_new_fibril))
        VV[odd][3] = oslope_new_fibril
        if VV[odd][2] == 30:
            VV[odd][2] = 1
        elif VV[odd][2] == 33:
            oend_second_fibril = odd
    
        if VV[odd][0] > 0:            
            VV[odd][0] = VV[odd][0]%boxSide
        if VV[odd][0] < 0:
#            print("go one lower than box!", djj[0])
            VV[odd][0] = boxSide - abs(VV[odd][0])%boxSide
        if VV[odd][1] > 0:
            VV[odd][1] = VV[odd][1]%boxSide
        if VV[odd][1] < 0:
#            print("go one higher than box!", djj[1])
            VV[odd][1] = boxSide - abs(VV[odd][1])%boxSide    
    
    second_fibril_association14 = [oxa1 for oxa1 in second_fibril_association14 if not oend_second_fibril in oxa1] 
    second_fibril_dissociation14 = [oxb1 for oxb1 in second_fibril_dissociation14 if oxb1 != oend_second_fibril in second_fibril_dissociation14] 
...
#                break
                return (VV)
            elif otest_dist_detach == round(2.0*sigma,13) and osk1[3] != oslope_new_fibril and VV[ojb1][2]==1:
                ofree_secondary_site.append([ojb1]) # identify the ewly freed fibril inside monomer for a secondary monomer attach
            elif osk1[2] in [31,32,33]:
                otmp_fibril_dissociation14.append(osk1)
                if ojb1 == 0:
                    otmp_fibril_association14.append([ojb1,osk1])
            elif osk1[2] == 1 and ojb1 == 0:
                otmp_second_monomer_attach14.append([ojb1,osk1])

    for ohg in ofree_secondary_site:
        for ojn in range(len(VV)):
            odist_new_secondary = dist(VV[ohg], VV[ojn])
            if odist_new_secondary == round(2.0*sigma,13):
                otmp_second_monomer_attach14.append([ojn,ohg])
    if otest_dist_second_detach == False:          
        ...
        if len(ofibril_new) == 5:
            pentamer_on_diffusion14.append(ofibril_new)
